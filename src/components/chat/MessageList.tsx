  ...
  if (messages.length === 0) {
    return (
      <div className="flex items-center justify-center h-full px-4">
        <div className="w-full max-w-md bg-white border border-neutral-200 rounded-xl shadow-lg p-8 text-center">
          <div className="flex items-center justify-center w-16 h-16 rounded-2xl bg-blue-50 mb-5 shadow-sm mx-auto">
            <Bot className="h-8 w-8 text-blue-600" />
          </div>
          <h2 className="text-neutral-900 font-bold text-xl mb-3">Start a conversation to generate React components</h2>
          <p className="text-neutral-600 text-base leading-relaxed">I can help you create buttons, forms, cards, and more</p>
        </div>
      </div>
    );
  }
                      ...
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "tool-invocation":
                            ...
                            // Map technical tool names to user-friendly labels
                            const toolNameMap: Record<string, string> = {
                              'str_replace_editor': 'Editing file',
                              'file_manager': 'Managing files'
                            };
                            const friendlyToolName = toolNameMap[tool.toolName] || tool.toolName;
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{friendlyToolName}</span>
                                  </>
                                ) : (
                                  <>
                          ...
                          case "source-document":
                            return (
                              <div key={partIndex} className="mt-2 text-xs text-neutral-500">
                                Source: {JSON.stringify((part as any).source || (part as any).url || (part as any).document)}
                              </div>
                            );
                          case "step-start":
